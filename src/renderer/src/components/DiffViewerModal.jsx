...
export default function DiffViewerModal({
  ...
  return (
    <div className={`fixed inset-0 backdrop-blur-md z-50 flex items-center justify-center p-4 ${
      theme === 'dark' ? 'bg-black/90' : 'bg-black/40'
    }`}>
      <div className={`w-full max-w-2xl border rounded-2xl p-4 shadow-2xl space-y-3 flex flex-col max-h-[85vh] animate-in zoom-in-95 duration-150 ${
        ...
        <div className={`flex items-center justify-between border-b pb-3 shrink-0 ${
          ...
          <div className="flex items-center gap-2 min-w-0">
            ...
            <h3 className={`text-xs font-bold truncate ${theme === 'dark' ? 'text-white' : 'text-zinc-900'}`}>
              {diffModal.prevCommit ? (
                <span className="flex items-center gap-1.5 flex-wrap">
                  ...
                  <span>➔</span>
                  <span className={`px-1.5 py-0.5 rounded font-mono text-[10px] ${
                    theme === 'dark' ? 'bg-zinc-800 text-white' : 'bg-black text-white'
                  }`}>
          ...
          <button 
            ...
            className={`text-xs font-bold px-2 py-1 rounded transition-colors shrink-0 ${
              theme === 'dark' ? 'text-zinc-400 hover:text-white bg-zinc-800' : 'text-zinc-500 hover:text-zinc-900 bg-zinc-100'
            }`}
          >
            ✕
          </button>
        ...
        {diffModal.loading ? (
          <div className={`py-16 text-center text-xs ${theme === 'dark' ? 'text-zinc-400' : 'text-zinc-500'}`}>
            Computing line-by-line diff...
          </div>
        ) : diffModal.isBinary ? (
          <div className="py-12 text-center space-y-3">
            <FileCheck className={`w-10 h-10 mx-auto ${theme === 'dark' ? 'text-zinc-600' : 'text-zinc-400'}`} />
            <p className={`text-xs font-semibold ${theme === 'dark' ? 'text-zinc-300' : 'text-zinc-800'}`}>Binary or Media Revision File</p>
            <p className={`text-[11px] max-w-xs mx-auto ${theme === 'dark' ? 'text-zinc-500' : 'text-zinc-500'}`}>
          ...
          <div className="flex-1 flex flex-col min-h-0 space-y-2">
            ...
            {diffModal.diffData && (
              <div className={`flex items-center justify-between px-3 py-2 rounded-xl border text-xs shrink-0 ${
                theme === 'dark' ? 'bg-black border-zinc-800' : 'bg-zinc-50 border-zinc-200'
              }`}>
                ...
                <span className={`text-[11px] font-mono ${theme === 'dark' ? 'text-zinc-400' : 'text-zinc-500'}`}>
            ...
            {/* Diff Code Container */}
            <div className="flex-1 overflow-y-auto bg-black rounded-xl border border-zinc-800 p-3 font-mono text-[11px] leading-relaxed space-y-0.5 select-text shadow-inner">
              {diffModal.diffData?.diff.map((line, idx) => (
                <div
                  ...
                  className={`px-2 py-0.5 rounded flex items-start space-x-2 ${
                    ...
                    line.type === 'delete' ? 'bg-red-950/40 text-red-300 border-l-2 border-red-500' :
        ...
        <div className={`pt-2 border-t flex items-center justify-between shrink-0 ${
          ...
          <span className={`text-[10px] font-mono ${theme === 'dark' ? 'text-zinc-500' : 'text-zinc-400'}`}>
